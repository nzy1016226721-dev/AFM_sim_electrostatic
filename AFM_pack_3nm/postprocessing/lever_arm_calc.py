...
def main():
    ...
    if qd_block is None:
        ...
        qd_block = {
            'x_range_nm': [0.25 * Lx, 0.75 * Lx],
            'y_range_nm': [0.25 * Ly, 0.75 * Ly],
            'z_range_nm': [max(0.0, Lz - 6.0), Lz]
        }
    else:
    ...
    for fpath in phi_files:
        ...
        tip_z_nm_cfg = cfg.get("tip_z_nm")
        if tip_z_nm_cfg is not None:
            tip_spacing_nm = float(tip_z_nm_cfg) - qd_top_z_nm
        else:
