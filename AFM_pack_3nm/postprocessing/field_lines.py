...
import glob
try:
    from ..simulation.coordinates import normalize_config, physical_domain_nm
except ImportError:
    from simulation.coordinates import normalize_config, physical_domain_nm

try:
    from ..simulation.solver import build_downward_pointing_tip
except ImportError:  # direct execution from afm_package/
    from simulation.solver import build_downward_pointing_tip
...
def plot_field_lines(phi, plane='xz', coord=0.5,
                      ...
                      min_arrow_len_nm=0,
                      mag_percentile=97):
    """Plot electric-field lines for a saved electrostatic potential."""
    ...
    if all(k in tip_params for k in ("tip_z_nm", "R_nm", "r_tip_nm", "domain_nm")):
        tip_mask, _, _ = build_downward_pointing_tip(
            nx, ny, nz,
            aspect_ratio=tip_params['aspect_ratio'],
            verbose=False,
            tip_z_nm=tip_params["tip_z_nm"],
            R_nm=tip_params["R_nm"],
            r_tip_nm=tip_params["r_tip_nm"],
            domain_nm=tip_params["domain_nm"],
            center_fraction=tip_params.get("origin_fraction", (0.5, 0.5, 0.0)),
        )
    else:
        tip_mask, _, _ = build_downward_pointing_tip(
            nx, ny, nz,
...
def interactive_main():
    """Run the interactive electric-field-line plotting interface."""
    ...
    for entry in detected:
        ...
        if entry['config_path']:
            ...
            Lx_nm, Ly_nm, Lz_nm = physical_domain_nm(raw_cfg)
            ...
            tip_params = {
                ...
                'origin_fraction': physical.get('origin_fraction', (0.5, 0.5, 0.0)),
            }
